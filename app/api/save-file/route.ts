...
export async function POST(request: NextRequest) {
  try {
    ...
    const smbCommand = `smbclient "//192.168.40.145/OBS Multi" -U guest% -d 3 -c "put ${tempFile} src/App.svelte"`;
    ...
    try {
      ...
      try {
        ...
        await execAsync(`smbclient "//192.168.40.145/OBS Multi" -U guest% -c "put ${tempFile} src/App.svelte"`);
